...
def pack_ret(func):
    def _(*args, **kwargs):
        ...
        try:
            ...
            endtm = time.time()
            conn = args[0]
            log.info('server=%s|func=%s|index=%s|dsl=%s|time=%d|ret=%s', conn.host, func.__name__, conn.index, conn.dsl, int((endtm-starttm)*1000000), ret)
            return ret
    return _
...
class ESquery():
    ...
    @pack_ret
    def es_create_index(self, index_name, ignore=[]):
    ...
    def __query_build(self, dsl, qfrom, qsize, includes, excludes, sort):
        dsl["_source"] = {
            "includes": includes,
            "excludes": excludes,
        }
    ...
    def es_query(self, index_name, dsl, qfrom, qsize, includes, excludes, sort):
        self.__query_build(dsl, qfrom, qsize, includes, excludes, sort)
        self.index = index_name
        self.dsl = dsl
    ...
    '''
    @query_pack_ret
    def es_query_match_bool(self, index_name, key='must', cond=[], qfrom=None, qsize=None, includes=[], excludes=[], sort=None):
        dsl = {
            "query": {
                "bool": {
                    key: cond,
