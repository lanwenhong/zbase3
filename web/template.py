...
class Render:
    def __init__(self, loc="template", tmpdir=None, cache=False, charset='utf-8'):
        self.loc = loc
        if cache:
            self.cache = {}
        else:
...
def install(loc="template", tmpdir="/tmp", cache=False, charset='utf-8'):
    global render
...
def test():
    ...
    #loc = os.path.join(path, 'template')
    loc = '/Users/apple/projects/python/xx8xx8/web/template/default/admin'
    r = Render(loc, 'tmp')
